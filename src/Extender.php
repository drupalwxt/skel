...
/**
 * Helper class to get information from a site's skel.extend.yml file.
 */
class Extender {
  ...
  public function getInfo() {
    // Discover skel.extend.yml first in the `sitePath`, and then defer to
    // `sites/all`.
    $paths[] = $this->sitePath . '/skel.extend.yml';
    $paths[] = $this->root . '/sites/all/skel.extend.yml';
  ...
  /**
   * Returns the list of Skeleton Extensions to enable.
   *
   * @return string[]
   *   The modules to enable.
   */
  public function getOpenDataExtensions() {
    $info = $this->getInfo();
    // Return FALSE instead of empty array because empty array means "don't
    // enable _any_ extensions" in this case.
    return isset($info['skel_extensions']) ? $info['skel_extensions'] : FALSE;
  }
