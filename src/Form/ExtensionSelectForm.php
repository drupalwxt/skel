...
use Drupal\skel\Extender;
use Drupal\skel\FormHelper;
use Symfony\Component\DependencyInjection\ContainerInterface;

/**
 * Defines a form for selecting which Skeleton extensions to install.
 */
class ExtensionSelectForm extends FormBase {

  /**
   * The Skeleton extender configuration object.
   *
   * @var \Drupal\skel\Extender
   */
  protected $extender;
   ...
   * The form helper.
   *
   * @var \Drupal\Skeleton\FormHelper
   */
   ...
   *
   * @param \Drupal\skel\Extender $extender
   *   The Skeleton extender configuration object.
   * @param string $root
   *   The Drupal application root.
   ...
   * @param \Drupal\skel\FormHelper $form_helper
   *   The form helper.
   */
  ...
  public static function create(ContainerInterface $container) {
    return new static(
      $container->get('skel.extender'),
      $container->get('app.root'),
      $container->get('info_parser'),
      $container->get('string_translation'),
      $container->get('skel.form_helper')
    );
  }
  ...
  /**
   * Yields info for each of Skeleton's extensions.
   */
  ...
  public function buildForm(array $form, FormStateInterface $form_state, array &$install_state = NULL) {
    ...
    $chosen_ones = $this->extender->getOpenDataExtensions();
    if (is_array($chosen_ones)) {
      ...
      // Explain ourselves.
      $form['help']['#markup'] = $this->t("Skeleton extensions have been pre-selected in the skel.extend.yml file in your sites directory and are disabled here as a result.");
    }
    else {
      $form['help']['#markup'] = $this->t("You can choose to disable some of Skeleton's functionality below. However, it is not recommended.");
    }
  ...
  /**
   * Forces the Skeleton Core extension to be selected.
   *
   * Turns the Skeleton Core checkbox into a persistent server-side value so
  ...
  public static function requireCore(array $element) {
    $element['skel_core'] = [
      '#type' => 'value',
      '#value' => $element['skel_core']['#return_value'],
    ];
    return $element;
  }
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $GLOBALS['install_state']['skel']['modules'] = array_merge($modules, $this->extender->getModules());
  }
